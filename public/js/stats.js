...
$('#stats-menu-panel').on('show', function(){
    var accumulatedJobTimeMS = parseInt((lw.store.get("accumulatedJobTimeMS") || 0),  10);
...
function accumulateTime(elapsedTimeMS) {
	...
	lw.store.set("accumulatedJobTimeMS", accumulatedJobTimeMS.toString());
...
function resetAccumulatedTime() {
	lw.store.set("accumulatedJobTimeMS", "0");
	var zero = 0;
