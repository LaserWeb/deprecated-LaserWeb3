...
$(document).ready(function() {
    ...
    $('#generategcode').on('click', function() { // DXF job Params to MC
        ...
        for (j = 5; j < scene.children.length; j++) {
            ...
            if (scene.children[j].name != 'object') {
              if (scene.children[j].type == "Mesh") {
                console.log('Object '+j+' is a Raster')
                runRaster(scene.children[j].userData.seq)
              } else {
                var cutSpeed0 = parseFloat( $("#speed"+(j-5)).val() ) * 60;
                var pwr0 = parseFloat( $("#power"+(j-5)).val() );
                rapidSpeed = parseFloat(document.getElementById('rapidspeed').value) * 60;
                g += "; \n";
                g += "; "+scene.children[j].name + "\n";
                g += "; \n";
                g += generateGcode(scene.children[j], cutSpeed0, pwr0, rapidSpeed, laseron, laseroff);
              }
            }
        }
